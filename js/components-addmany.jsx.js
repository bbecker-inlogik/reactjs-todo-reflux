...
(function(React, TodoActions, global) {
    ...
    var AddMany = React.createClass({
        ...
        handleEditStart: function(evt) {
            ...
            this.setState({
                ...
                this.refs.addMany.getDOMNode().focus();
            });
        },
        handleAddMany: function() {
            debugger;
            var manytext = this.refs.addMany.getDOMNode().value;
            var addManyList = manytext.split('\n');
            _.map(addManyList, function(item) {
                TodoActions.addItem(item);
            });
            //TodoActionAddMany.addMany();
        },
        render: function() {
            return (
                <div id="addmany">
                    <textarea ref="addMany" ></textarea><br />
                    <a onClick={this.handleAddMany}>Add Many</a>
                </div>
            );
        }
    });
    ...
    React.render(
        <AddMany />,
        document.getElementById('addmanycontainer')
    );
